...
//     //timer
// }
static lv_obj_t * ui_HM;
static lv_obj_t * ui_DD;
static lv_obj_t * label_scroll_value;
static void scroll_event_cb(lv_event_t * e) {
    lv_obj_t * obj = lv_event_get_target(e);
    lv_coord_t y = lv_obj_get_scroll_y(obj); // 获取Y轴的滚动值
    
    char buf[32];
    snprintf(buf, sizeof(buf), "Scroll Value: %d", y); // 格式化滚动值
    lv_label_set_text(label_scroll_value, buf); // 更新文本标签
    if (lv_event_get_code(e) == LV_EVENT_SCROLL_END) {
        if (y <= 70) {
            lv_obj_scroll_to_y(ui_DD, 0, LV_ANIM_OFF); // 滚动到顶部
        } else if (y > 70) {
            lv_obj_scroll_to_y(ui_DD, 126, LV_ANIM_OFF); // 滚动到指定位置
        }
    }
    else{
      lv_obj_scroll_to_y(ui_DD, 126, LV_ANIM_OFF);
    }
}

// static void test_ui_init(void)
// {
//   ui_HM = lv_obj_create(NULL);

//   ui_DD = lv_obj_create(ui_HM);
//   lv_obj_set_width(ui_DD, 240);
//   lv_obj_set_height(ui_DD, 420);
//   lv_obj_set_x(ui_DD, 0);
//   lv_obj_set_y(ui_DD, -10);
//   lv_obj_set_align(ui_DD, LV_ALIGN_TOP_MID);// 作用：居中
  
//   // lv_obj_scroll_by(ui_DD,0,-126,LV_ANIM_OFF);
//   lv_obj_clear_flag(ui_DD, LV_OBJ_FLAG_SCROLL_ELASTIC | LV_OBJ_FLAG_SCROLL_CHAIN);      /// Flags
//   //lv_obj_clear_flag(ui_DropDownPanel, LV_OBJ_FLAG_CLICKABLE | LV_OBJ_FLAG_SCROLL_ELASTIC | LV_OBJ_FLAG_SCROLL_CHAIN);      /// Flags
//   lv_obj_set_scroll_dir(ui_DD, LV_DIR_VER);// 设置滚动方向 LV_DIR_VER：垂直滚动 LV_DIR_HOR：水平滚动
  

//   lv_obj_t * ui_UB = lv_obj_create(ui_DD);
//   lv_obj_set_width(ui_UB, 240);
//   lv_obj_set_height(ui_UB, 130);
//   lv_obj_set_x(ui_UB, 0);
//   lv_obj_set_y(ui_UB, -10);
  

//   lv_obj_set_align(ui_UB, LV_ALIGN_TOP_MID);
//   lv_obj_clear_flag(ui_UB, LV_OBJ_FLAG_CLICKABLE | LV_OBJ_FLAG_SCROLLABLE);      /// Flags
//   static lv_style_t style_red;
//   lv_style_init(&style_red);
//   lv_style_set_bg_color(&style_red, lv_color_hex(0xFF0000));
//   lv_obj_add_style(ui_UB, &style_red, 0);

//   lv_obj_t * ui_DB = lv_obj_create(ui_DD);
//   lv_obj_set_width(ui_DB, 240);
//   lv_obj_set_height(ui_DB, 130);
//   lv_obj_set_x(ui_DB, 0);
//   lv_obj_set_y(ui_DB, 420);
//   lv_obj_set_align(ui_DB, LV_ALIGN_TOP_MID);
//   lv_obj_clear_flag(ui_DB, LV_OBJ_FLAG_CLICKABLE | LV_OBJ_FLAG_SCROLLABLE);      /// Flags

//   label_scroll_value = lv_label_create(ui_HM); // 在ui_HM上创建一个文本标签
//   lv_coord_t y = lv_obj_get_scroll_y(ui_DD);
//   char buf[32];
//   snprintf(buf, sizeof(buf), "Scroll Value: %d", y);
//   lv_label_set_text(label_scroll_value, buf); // 更新文本标签
//   lv_obj_align(label_scroll_value, LV_ALIGN_TOP_LEFT, 0, 0); // 对齐文本标签

//   // 为ui_DD添加滚动事件监听器
//   lv_obj_add_event_cb(ui_DD, scroll_event_cb, LV_EVENT_SCROLL_END, NULL);
